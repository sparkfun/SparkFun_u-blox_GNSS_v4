 ...
 *
 * ESP-IDF version of the Arduino example PollingExample4_SECUNIQID.ino
 *
 ...
 * The helper method getUniqueChipIdStr() converts the bytes to a hexadecimal string
 * (std::string on ESP-IDF, String on Arduino).
 *
...
extern "C" void app_main(void)
{
    ...
    {
        std::string uniqueId = myGNSS.getUniqueChipIdStr();
        printf("Unique chip ID: 0x%s\n", uniqueId.c_str());
    }
    else
    {
        printf("getSECUNIQID failed!\n");
    }

    // Nothing more to do. Returning from app_main is OK: the main task is deleted.
}
