 ...
 *
 * ESP-IDF version of the Arduino example PeriodicExample2_GPGGA.ino
 *
...
extern "C" void app_main(void)
{
    ...
    while (true)
    {
        ...
        if (myGNSS.getNMEA("GGA"))
        {
            ...
            printf(" Lat: %s", myGNSS.getNmeaMessageField(msg, "lat").c_str()); // Print the latitude
            printf(" %s", myGNSS.getNmeaMessageField(msg, "NS").c_str());       // Print the North / South indicator
            ...
            printf(" %s (degrees)", myGNSS.getNmeaMessageField(msg, "EW").c_str());  // Print the East / West indicator

            printf(" Alt: %s (m)\n", myGNSS.getNmeaMessageField(msg, "alt").c_str()); // Altitude above MSL
        }
