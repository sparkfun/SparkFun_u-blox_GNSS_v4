...
void printESFRAWdata(ubxCallbackDataCommon_t *theData)
{
  ...
  if (1) // Change to 0 to disable the callback prints. Useful when trying to capture debug...
  {
    ...
    // Call getUbxMessageBlockCount() - to confirm this works on ESF RAW
    uint8_t numMeas = myGNSS.getUbxMessageBlockCountCallback(msg);
    ...
    for (uint8_t i = 0; i < numMeas; i++)
    {
      ...
      // Print the sTag sensor time tag
      uint32_t sTag = (uint32_t)myGNSS.getUbxMessageBlockFieldCallback(msg, i, "sTag");
...
void setup()
{
  ...
  // Enable the ESF RAW Message on I2C, every navigation cycle
  myGNSS.setCfgValset(UBLOX_CFG_MSGOUT_UBX_ESF_RAW_I2C, 1);

  // Set up a callback for ESF RAW messages. Call newSECSIG() each time one arrives.
  ...
  myGNSS.setAutoCallbackPtr("ESF", "RAW", &printESFRAWdata);
}
